...
class Monitor {
     ...
     */
    Monitor(int maxTime) {
ReentrantLock lock0 = new ReentrantLock(true);lock0.lock();        now = 0;
        waitList = new MyLinkedList();
lock0.unlock();        max = maxTime;
    }
    ...
    void tick() {
		synchronized (this) {
ReentrantLock lock1 = new ReentrantLock(true);lock1.lock();        now++;

        if (!waitList.isEmpty()) {
ReentrantLock lock2 = new ReentrantLock(true);lock2.lock();            MyObject first = (MyObject) waitList.firstElement();
